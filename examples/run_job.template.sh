...
# Replace this block with your actual evaluation command.
# Example: bash eval.sh --config config.yaml --weights model.safetensors
{
  echo "starting eval for $JOB_ID at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
  sleep 2
  echo "finished eval for $JOB_ID at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
